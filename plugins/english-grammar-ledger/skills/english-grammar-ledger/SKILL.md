...
This skill handles two independent logging tasks, triggered by different conditions in the user prompt.
...
Detect natural-language Chinese text in user messages. Ignore code, command lines, paths, identifiers, quoted tool output, and intentional fragments.
...
- Both tasks run independently — both can fire on the same user message if applicable.

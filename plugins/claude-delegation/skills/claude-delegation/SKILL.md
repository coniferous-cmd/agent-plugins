...
Use `--system` for task-wide instructions, `--model` to select a CLI model alias, and `--max-turns` to bound agentic work. Pipe a prompt through stdin only when `--prompt` is omitted.
